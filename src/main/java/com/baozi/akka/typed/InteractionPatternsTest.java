...
public class InteractionPatternsTest {
  ...
  interface Samples {
    ...
    public class CookieFabric {
      ...
      void demo() {
        ActorRef<Request> cookieFabric = null;
        ActorContext<Response> context = null;

        // #request-response-send
        cookieFabric.tell(new Request("give me cookies", context.getSelf()));
        // #request-response-send
      }
    }
  ...
  public void fireAndForgetSample() throws Exception {
    ...
    final ActorSystem<Samples.Printer.PrintMe> system =
        ActorSystem.create(Samples.Printer.create(), "printer-sample-system");

    // note that system is also the ActorRef to the guardian actor
    final ActorRef<Samples.Printer.PrintMe> ref = system;

    // these are all fire and forget
    ref.tell(new Samples.Printer.PrintMe("message 1"));
    ref.tell(new Samples.Printer.PrintMe("message 2"));
    // #fire-and-forget-doit
  ...
  public void timers() throws Exception {
    TestProbe<Samples.Buncher.Batch> probe = testKit.createTestProbe(Samples.Buncher.Batch.class);
    ActorRef<Samples.Buncher.Command> buncher =
        testKit.spawn(Samples.Buncher.create(probe.ref(), Duration.ofSeconds(1), 10), "batcher");

    Samples.Buncher.ExcitingMessage msgOne = new Samples.Buncher.ExcitingMessage("one");
    Samples.Buncher.ExcitingMessage msgTwo = new Samples.Buncher.ExcitingMessage("two");
    buncher.tell(msgOne);
    buncher.tell(msgTwo);
    probe.expectNoMessage();
    probe.expectMessage(Duration.ofSeconds(2), new Samples.Buncher.Batch(Arrays.asList(msgOne, msgTwo)));
  }
